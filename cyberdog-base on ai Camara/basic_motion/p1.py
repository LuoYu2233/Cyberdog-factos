...
# 
        ...
        # 持续检查机器人前方约89度位置的障碍物距离。
        # 如果距离大于0.6米，机器人会向前移动一小段距离（0.2米/秒，持续1秒）。
        # 每次移动后，会再次检查距离。
        # 这个过程会一直重复，直到障碍物距离小于或等于0.6米
        print("持续检查距离准备转弯...")
        while Ctrl.getDistance(89) > 0.6:
            print("离墙体的距离："+ str(Ctrl.getDistance(89)))
            msg.mode = 11   
            msg.gait_id = 27
            msg.vel_des = [0.2,0,0]
        ...
        #切换到回退的步态
        print("倒走ing...")
        move_change('backward', msg, Ctrl, 1800, 0.9)
        time.sleep(1)
        Ctrl.fixAngle(msg)  # 居正
        Ctrl.navigate(msg)  # 回中
        move('right', msg, Ctrl, 560, 0.3)
        ...
        print("上坡狮子")  
        msg.life_count += 1
        while Ctrl.getDistance(89) != float('inf'):  
            print("未检测到顶端，继续前进...")  
            Ctrl.printDis(89)
            msg.mode = 11   
            msg.gait_id = 27  
...
class Robot_Ctrl(object):
    ...
    def printDis(self, angle):
        print("Front Distance:", self.laser_scanner.list[angle])
        return self.laser_scanner.list[angle]
