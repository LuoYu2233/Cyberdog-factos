...
def getAngle(self):
    ...
    该函数通过激光雷达扫描数据，先进行中值滤波去噪，然后将极坐标数据转换为笛卡尔坐标，
    生成二值图像后利用霍夫变换检测墙壁直线，最后计算机器人与墙壁的夹角（单位：度）。

    返回值：
        float: 机器人与墙壁的夹角（度），正负表示方向。
    """
    # 获取激光扫描数据  
...
def fixAngle(self, msg):
    ...
    该函数调用 getAngle() 获取当前与墙壁的夹角，根据角度大小和方向自动控制机器人左右转动，
    直到夹角小于阈值（默认2度）为止。调整过程中会递归微调，确保对齐。

    参数:
        msg: 机器人控制命令对象，调整时会修改其运动参数。
    """
    print("将调整角度")  
