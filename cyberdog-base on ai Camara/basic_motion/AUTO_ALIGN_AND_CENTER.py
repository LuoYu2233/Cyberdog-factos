# ===============================
# 文件名：AUTO_ALIGN_AND_CENTER.py
# 功能：基于激光雷达数据实现机器人自动居中与自动对齐
# 说明：本文件用于机器人在走廊等场景下，自动根据激光雷达数据调整自身位置和朝向，实现与墙体平行并居中。
# 包含：
#   1. 激光雷达数据订阅与处理
#   2. 距离与角度计算
#   3. 自动导航与姿态调整
...
class SensorSubscriber(Node):
    """
    激光雷达数据订阅节点。
    负责接收/scan话题的激光雷达数据，并进行无效点插值、左右距离提取等处理。
    """
    def __init__(self):
